...
SIMU_config := $(SIM_DIR)/$(notdir $(basename $(PROG)))

# Rules
mem_files: $(MEM_DIR)/prog.mem $(SIMU_config)
...
$(SIM_DIR)/%: %.c $(MEM_DIR)/%.mem
	...
	cp $(MEM_DIR)/$(PROG).mem $@/test_default.mem
	# Generate .irq
	@echo "" > $@/test_default.irq
	# generate .out
	@echo "" > $@/test_default.out
	# generate .setup
	@echo "100" > $@/test_default.setup
	@echo "false" >> $@/test_default.setup
	cp $@/* $(SIM_DIR)/
